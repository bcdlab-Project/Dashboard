        ...
        if (data['ok']) {
            ...
            data['users'].forEach(user => {
                ...
                row.innerHTML = 
                    ...
                    '<td class="hidden px-4 py-4 sm:table-cell whitespace-nowrap">' + user.role['name_<?=lang('Utilities.language')?>'] + '</td>' +
                    '<td class="hidden px-4 py-4 whitespace-nowrap lg:table-cell">' + (user.has_github ? '<span class="px-3 py-2 text-xs font-semibold text-green-600 bg-green-100 rounded-full dark:bg-emerald-400/50 dark:text-green-500"><?=lang('CustomTerms.connected')?></span>' : '<span class="px-3 py-2 text-xs font-semibold text-red-600 bg-red-100 rounded-full dark:bg-red-500/50 dark:text-rose-500/85" ><?=lang('CustomTerms.disconnected')?></span>') + '</td>' +
